...
jobs:
  ci:
    ...
    steps:
      ...
      - name: Trigger Upline deploy
        run: |
          STATUS=$(curl -s -o /dev/null -w "%{http_code}" \\
            -X POST "\${{ secrets.UPLINE_WEBHOOK_URL }}")
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 p-4" onClick={onClose}>
      ...
      >
        ...
        <div className="px-6 py-3 bg-zinc-950/40 border-b border-zinc-800 text-xs text-zinc-500 space-y-1">
          ...
          <p>2. Add a GitHub secret named <code className="text-violet-400">UPLINE_WEBHOOK_URL</code> with your webhook URL.</p>
        </div>
...
const AI_STEPS = [
  ...
  { icon: "⬡", label: "Wiring Upline webhook",    sub: "Injecting auto-deploy trigger into the pipeline..." },
];
